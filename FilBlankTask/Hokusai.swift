...
final public class HOKButton: UIButton {
    ...
    func setColor(_ colors: HOKColors) {
        self.setTitleColor(colors.fontColor, for: UIControl.State())
...
final public class Hokusai: UIViewController, UIGestureRecognizerDelegate {
    ...
    required public init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(Hokusai.onOrientationChange(_:)), name: UIDevice.orientationDidChangeNotification, object: nil)
    }
    ...
    public func show() {
        ...
        guard let colors = self.colors == nil ? colorScheme.getColors() : self.colors else {
